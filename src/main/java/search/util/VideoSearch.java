...
public class VideoSearch {
  ...
  public static String getTitleLink(String title, String year) throws Exception {
    ...
    for (int i = NUM_SEARCH_ENGINES; i > 0; i--) {
      ...
      try {
        String result = Regex.firstMatch(Connection.getSourceCode(searchEngine + encodedQuery, DomainType.SEARCH_ENGINE, Constant.MS_2DAYS), 619);
  ...
  public static String getSummary(String sourceCode, boolean isTVShow) {
    ...
    boolean linkSummary2 = (summary2.isEmpty() && !Str.get(798).isEmpty());
    if (linkSummary2) {
      summary2 = "&#8203;";
    }
    ...
    return summary.insert(0, "<html><head><title></title></head><body><table><tr><!--poster-->").append("</table></body></html>").toString();
  }

  private static void getNames(String names, String type, Collection<StringBuilder> nameLists) {
    List<String> namesArr = Regex.matches(names, 198).stream().distinct().collect(Collectors.toCollection(ArrayList::new));
