...
public class RegularSearcher extends AbstractSearcher {
  ...
  public RegularSearcher(GuiListener guiListener, int numResultsPerSearch, Boolean isTVShow, Calendar startDate, Calendar endDate, String title, String[] genres,
    ...
    super(guiListener, numResultsPerSearch, isTVShow, !Boolean.FALSE.equals(isTVShow) ? Constant.MS_2DAYS : Constant.MS_3DAYS);
  ...
  protected void initialSearch() throws Exception {
    ...
    String sourceCode = Connection.getSourceCode(titleLink, DomainType.VIDEO_INFO, cacheExpirationMs);
  ...
  protected String getUrl(int page, boolean isTVShow, int numResultsPerSearch) {
    ...
    String urlTitle = (title.isEmpty() ? Str.get(142) : Str.get(10) + ThrowingSupplier.get(() -> URLEncoder.encode(Regex.htmlToPlainText(title),
            Constant.UTF8)));
  ...
  protected void addVideo(String titleMatch) {
    ...
    Video video = new Video(VideoSearch.normalize(Regex.firstMatch(Regex.match(titleMatch, 23), 628)), Regex.match(titleMatch, 25), Regex.firstMatch(
            yearAndType, 29), tvShow, Regex.isMatch(Regex.match(yearAndType, 796), tvShow ? 591 : 592));
  ...
  protected Video update(Video video) throws Exception {
    video.summary = Connection.getSourceCode(VideoSearch.url(video), DomainType.VIDEO_INFO, cacheExpirationMs);
    return video;
  }
